...
diabetes_df = diabetes_df[diabetes_df$DIQ010 != 3 & diabetes_df$DIQ010 != 7 & diabetes_df$DIQ010 != 9, ] %>% 
 mutate(RIAGENDR = as_factor(RIAGENDR), RIDRETH3 = as_factor(RIDRETH3), DIQ010 = as_factor(DIQ010) ) %>% 
  drop_na(DIQ010) 
 
 colnames(diabetes_df) = c("ID", "gender", "age", "race", "bmi", "fasting_hrs", "workload", "hdl", "ldl", "triglyceride", "insulin", "hr2_glucose_tolerance", "glucose_challenge", "glucose_tolerance", "glucose", "diabetes") 
 
 levels(diabetes_df$diabetes)[1] <- "yes"
 levels(diabetes_df$diabetes)[2] <- "no"
```
